        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                if (mysqli_stmt_affected_rows($stmt) > 0) {
                    ...
                    $response['message'] = 'No reservation found with the given ID or status is already the same.';
                }
            } else {
                $response['message'] = 'Database error: Could not update reservation status.';
                error_log("Update reservation status error: " . mysqli_stmt_error($stmt));
            }
