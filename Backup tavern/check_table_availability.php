...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- IMPORTANT: HYPOTHETICAL TOTAL RESTAURANT CAPACITY ---
    // In a real application, you would fetch this from a 'tables' or 'restaurant_settings' table
    // or calculate it based on the sum of capacities of all your tables.
    // For this example, we use a fixed value.
    $TOTAL_RESTAURANT_CAPACITY = 50; // Example: The restaurant can seat a total of 50 guests at any given time.

    $booked_guests_for_slot = 0;

    // Fetch existing confirmed or pending reservations for the specific date and time
    // We assume 'Confirmed' and 'Pending' reservations occupy tables.
    $sql = "SELECT SUM(num_guests) AS total_booked_guests FROM reservations WHERE res_date = ? AND res_time = ? AND (status = 'Confirmed' OR status = 'Pending')";
